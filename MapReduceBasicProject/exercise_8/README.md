...
## Description
This exercise implements a MapReduce job for income monthly totals.

## Files
- `IncomeMonthlyTotalsDriver` - MapReduce Driver class
...
hdfs dfs -rm -r /user/$USER/exercise_8/input 2>/dev/null
hdfs dfs -rm -r /user/$USER/exercise_8/output 2>/dev/null

# Create input directory
hdfs dfs -mkdir -p /user/$USER/exercise_8/input

# Upload input file(s)
cd exercise_8
hdfs dfs -put input.txt /user/$USER/exercise_8/input/
```
...
hadoop jar target/MapReduceProject-1.0.0.jar \
  it.polito.bigdata.hadoop.income.monthlytotals.IncomeMonthlyTotalsDriver \
  2 \
  /user/$USER/exercise_8/input \
  /user/$USER/exercise_8/output
```

### 3. View Results
```bash
# List output files
hdfs dfs -ls /user/$USER/exercise_8/output

# View results
hdfs dfs -cat /user/$USER/exercise_8/output/part-r-00000

# View all output files (if multiple reducers)
hdfs dfs -cat /user/$USER/exercise_8/output/part-r-* | sort
```
