...
## Description
This exercise implements a MapReduce job for inverted index.

## Files
- `InvertedIndexDriver` - MapReduce Driver class
...
hdfs dfs -rm -r /user/$USER/exercise_7/input 2>/dev/null
hdfs dfs -rm -r /user/$USER/exercise_7/output 2>/dev/null

# Create input directory
hdfs dfs -mkdir -p /user/$USER/exercise_7/input

# Upload input file(s)
cd exercise_7
hdfs dfs -put input.txt /user/$USER/exercise_7/input/
```
...
hadoop jar target/MapReduceProject-1.0.0.jar \
  it.polito.bigdata.hadoop.invertedindex.InvertedIndexDriver \
  2 \
  /user/$USER/exercise_7/input \
  /user/$USER/exercise_7/output
```

### 3. View Results
```bash
# List output files
hdfs dfs -ls /user/$USER/exercise_7/output

# View results
hdfs dfs -cat /user/$USER/exercise_7/output/part-r-00000

# View all output files (if multiple reducers)
hdfs dfs -cat /user/$USER/exercise_7/output/part-r-* | sort
```
