...
## Description
This exercise implements a MapReduce job for pm10 average.

## Files
- `PM10AverageDriver` - MapReduce Driver class
...
hdfs dfs -rm -r /user/$USER/exercise_5/input 2>/dev/null
hdfs dfs -rm -r /user/$USER/exercise_5/output 2>/dev/null

# Create input directory
hdfs dfs -mkdir -p /user/$USER/exercise_5/input

# Upload input file(s)
cd exercise_5
hdfs dfs -put input.txt /user/$USER/exercise_5/input/
```
...
hadoop jar target/MapReduceProject-1.0.0.jar \
  ...
  2 \
  /user/$USER/exercise_5/input \
  /user/$USER/exercise_5/output
```

### 3. View Results
```bash
# List output files
hdfs dfs -ls /user/$USER/exercise_5/output

# View results
hdfs dfs -cat /user/$USER/exercise_5/output/part-r-00000

# View all output files (if multiple reducers)
hdfs dfs -cat /user/$USER/exercise_5/output/part-r-* | sort
```
